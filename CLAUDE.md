...
# Run bash function unit tests only
just test-unit

# Spin up interactive Docker dev environment
just dev

# Rebuild Docker image (run after Dockerfile changes)
just build
...
- **Shell function bugs**: add a case to the relevant `tests/test_*.sh` script
