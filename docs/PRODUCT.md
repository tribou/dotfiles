...
- **Fast shell startup**: Lib scripts and mise loading is optimized to minimize prompt latency
...
- Shell startup time remains fast (--no-use` for performance; plugins are lazy-loaded) `--no-use`; plugins are lazy-loaded)
