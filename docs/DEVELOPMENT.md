...
- --no-use` for performance; Node auto-switches on directory change via mise (replaces nvm) `--no-use` for performance; Node auto-switches on directory change via PROMPT_COMMAND
