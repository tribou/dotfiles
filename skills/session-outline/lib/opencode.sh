...
oc_q() { sqlite3 -readonly -noheader -separator "$(printf '\t')" "$oc_db" "$1"; }
...
backend_resolve() {
  ...
  if [[ -n $arg ]]; then
    ...
    oc_session=$(oc_q "SELECT s.id FROM session s JOIN project p ON p.id = s.project_id
                       WHERE p.worktree = '$(pwd)' AND s.parent_id IS NULL
...
backend_header() {
  ...
  title=$(oc_q "SELECT COALESCE(title,'') FROM session WHERE id = '$oc_session';")
  ...
  oc_q "
    WITH rows AS (
      SELECT
        ...
        trim(replace(replace(COALESCE(json_extract(p.data,'\$.state.input.description'),''),
             char(10),' '), char(9),' ')) AS descr,
...
oc_emit() {
  ...
  while IFS=$'\t' read -r kind text model child; do
    printf '%s\t%s\t%s\t%s\n' "$depth" "$kind" "$text" "$model"
    [[ -n ${child:-} ]] || continue
    oc_emit "$((depth + 1))" "$child" | awk -F'\t' '$2 != "PROMPT"' || true
